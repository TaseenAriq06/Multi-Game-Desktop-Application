...
def rock_paper_scissors():
    ...
    def finish_turn(player_choice, computer):
        ...
        score_text = f"Player: {score['Player']} | Computer: {score['Computer']} | Ties: {score['Ties']}"
        result_label.config(
    ...
    tk.Button(root, text="Rock 🪨", bg="gray", fg="black", font =("Georgia", 15), command=lambda: play("Rock")).place(x=00, y=200)
    tk.Button(root, text="Paper 📃", bg="lightgreen", fg="black", font =("Georgia", 15), command=lambda: play("Paper")).place(x=145, y=200)
    tk.Button(root, text="Scissors ✂️", bg="purple", fg="white", font =("Georgia", 15), command=lambda: play("Scissors")).place(x=285, y=200)
    tk.Button(root, text="Quit ❌", bg="red", fg="black", font =("Georgia", 15), command=root.destroy).place(x=147, y=250)
    tk.Button(root, text="Fire 🔥", bg="orange", fg="black", font =("Georgia", 15), command=lambda: play("Fire")).place(x=5, y=250)
    tk.Button(root, text="Water 💦", bg="blue", fg="white", font =("Georgia", 15), command=lambda: play("Water")).place(x=290, y=250)
